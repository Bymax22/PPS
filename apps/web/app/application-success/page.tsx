...
function ApplicationSuccessContent() {
  ...
  async function pay() {
    ...
    try {
      const res = await fetch(`/api/admissions/${id}/payment`, {
        method: 'POST',
      ...
      alert(
        'Payment initiated. Follow the instructions to complete payment.'
      )
    } catch (err: any) {
      ...
      <div className="container mx-auto px-6 max-w-3xl">
        ...
        >
          ...
          <div className="bg-gray-50 rounded-xl p-6 mb-8">
            ...
            <p className="text-3xl font-mono font-bold text-[#003087]">
              {admission?.id
                ? `APP-${admission.id.slice(0, 8).toUpperCase()}`
                : 'Processing...'}
            </p>
          ...
          <div className="flex gap-3 justify-center mb-6">
            <button
              onClick={downloadPdf}
              className="px-4 py-2 bg-[#003087] text-white rounded"
            >
              Download PDF
            </button>
            ...
            <button
              onClick={pay}
              disabled={loading}
              className="px-4 py-2 bg-[var(--campus-gold)] rounded"
            >
              Pay Enrollment Fee
            </button>
          </div>
          ...
          {admission && (
            <div className="mt-6 p-4 bg-white rounded shadow">
              ...
              <p className="text-sm">
                Applicant: {admission.studentName}
              </p>
              <p className="text-sm">
                Applying for: {admission.applyingForGrade}
              </p>
              <p className="text-sm">
          ...
          <div className="space-y-4 mt-8">
            ...
            <ul className="text-left space-y-2">
              ...
              <li className="flex items-start gap-2">
                <CheckCircle className="w-5 h-5 text-green-500 mt-0.5" />
                <span className="text-sm text-gray-600">
                  Prepare for assessment if applicable
                </span>
              </li>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
            <Link
              href="/"
              className="inline-flex items-center justify-center gap-2 px-8 py-3 bg-[var(--campus-gold)] text-gray-900 rounded-xl font-semibold"
            >
            ...
            <Link
              href="/contact"
              className="inline-flex items-center justify-center gap-2 px-8 py-3 border-2 border-[#003087] text-[#003087] rounded-xl font-semibold"
            >
              Contact Admissions
