...
export default function AdminParentsPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage('Parent registered successfully.')
      setFirstName('')
      setLastName('')
      setEmail('')
      setPhone('')
    ...
    <div className="space-y-8 py-10">
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-[2rem] bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
          <div className="mb-6">
            ...
            <h1 className="mt-3 text-3xl font-semibold text-slate-900">Parent registration</h1>
            <p className="mt-2 text-sm leading-6 text-slate-600">Create guardian accounts to manage student progress and payments.</p>
          </div>
        ...
        <section className="rounded-[2rem] bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
          <div className="mb-6">
            ...
            <h2 className="mt-3 text-2xl font-semibold text-slate-900">Parents overview</h2>
          </div>
          <div className="grid gap-4">
            <div className="rounded-3xl bg-slate-50 p-4 text-sm text-slate-700">Total parents: {rows.length}</div>
          </div>
        </section>
      </div>
      ...
      <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
        <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Parents list</p>
            <h2 className="mt-2 text-xl font-semibold text-slate-900">All parent accounts</h2>
          </div>
