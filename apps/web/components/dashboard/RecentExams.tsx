  ...
  return (
    <div className="space-y-4">
      {examAttempts.slice(0, 3).map((attempt) => {
        ...
        return (
          <div key={attempt.id} className="p-4 rounded-lg bg-gray-50">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="font-semibold text-gray-900">{attempt.exam.title}</h3>
                <p className="text-sm text-gray-600">{attempt.exam.class?.name}</p>
              </div>
              {attempt.isPassed !== null && (
                <div 
                  className="px-2 py-1 rounded-full text-xs font-semibold text-white"
                  style={{ backgroundColor: attempt.isPassed ? '#0EF117' : '#003087' }}
                >
