  ...
  return (
    <div className="space-y-3">
      {resources.map((resource) => (
        <div key={resource.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {resource.cloudinaryUrl && (
            <div className="mt-3 flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] text-slate-400">{resource.downloadCount ?? 0} downloads</span>
            </div>
          )}
        </div>
      ))}
